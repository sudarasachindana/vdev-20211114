...
abstract class _FormStore with Store {
  ...
  Future login() async {
    ...
    Future.delayed(Duration(milliseconds: 500)).then((future) {

      bool isMatch = userMap.containsKey(userEmail);
      if(isMatch){
        if(userMap[userEmail] == password){
          loading = false;
          success = true;
        }else{
          loading = false;
          success = false;
          errorStore.errorMessage = "Password doesn't match";
        }
      }else{
        loading = false;
        success = false;
